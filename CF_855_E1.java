...
public class CF_Round_855 {
	...
	private static void solve() {
		...
		if (n <= 3) {
			...
			char[] ff = {s.charAt(0), s.charAt(3)};
			char[] ss = {t.charAt(0), t.charAt(3)};
			Arrays.sort(ff);
			Arrays.sort(ss);
			if (ff[0] == ss[0] && ff[1] == ss[1] && s.substring(1, 3).equals(t.substring(1, 3))) {
				fw.out.println("YES");
			} else {
				fw.out.println("NO");
			}
			...
			boolean flag = true;
			for (int i = 0; i < 4; i++) {
				if (ff[i] != ss[i]) flag = false;
			}
			if (flag && s.charAt(2) == t.charAt(2)) {
				fw.out.println("YES");
			} else {
				fw.out.println("NO");
			}
